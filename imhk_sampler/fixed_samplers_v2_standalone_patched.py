...
"""
Standalone patched samplers with all type errors fixed.
"""
...
def sanitize_numeric_input(value, name: str, expected_type=float):
    ...
    if isinstance(value, (list, tuple, np.ndarray)):
        if len(value) == 1:
            ...
            raise ValueError(f"{name} must be scalar, got {type(value)} with {len(value)} elements")
...
def _sample_standard_lattice_imhk(basis: matrix, 
    ...
    for i in range(total_steps):
        ...
        if np.random.random() < np.exp(float(log_ratio)):
            current = proposal
            if i >= burn_in:
                accepted += 1
